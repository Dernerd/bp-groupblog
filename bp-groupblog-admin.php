...
function bp_groupblog_update_defaults() {
	...
	if ( ($newoptions['redirectblog'] == 2) ) {
		...
		echo '<p><strong>Die folgenden Blogs wurden aktualisiert</strong></p>';
			...
			if ( $page_found ) {
				...
				echo '<p style="line-height: 16px;"><strong>Wir haben die folgenden Blogs übersprungen</strong></p>';
				foreach ( $exists_in as $blog ) {
					echo '<p>- ' . $blog . '</p>';
				}
				echo '<p><em>Diese Blogs hatten bereits eine Seite mit dem Namen <strong>"'. $newoptions["pagetitle"] .'"</strong> die nicht von uns erstellt wurde. Bitte überprüfe und lösche diese Seite dauerhaft. Danach sollten Sie hierher zurückkehren und erneut auf Speichern klicken, um den Vorgang abzuschließen. Alternativ kannst Du einen anderen Vorlagenseitennamen auswählen.</em></p></div>';
			}
...
function bp_groupblog_add_admin_menu() {
	...
	$page = add_submenu_page( 
		...
		$location, 
		__( 'GruppenBlog-Setup', 'groupblog' ), 
		'<span class="bp-groupblog-admin-menu-header">' . __( 'GruppenBlog-Setup', 'groupblog' ) . '&nbsp;&nbsp;&nbsp;</span>', 
		'manage_options', 
...
function bp_groupblog_management_page() {
	...
	if( is_super_admin() == false )
		wp_die( __( 'Du hast keine Berechtigung, auf diese Seite zuzugreifen.', 'groupblog' ) );
	...
	<?php if ($updated) { ?>
  	<div id="message" class="updated fade">
  		<p><?php _e( 'Optionen gespeichert.', 'groupblog' ) ?></p>
  	</div>
  <?php	} ?>

	<div class="wrap" style="position: relative">
		<h2><?php _e( 'BuddyPress GruppenBlog-Einstellungen', 'groupblog' ) ?></h2>
		...
		<form name="bp-groupblog-setup" id="bp-groupblog-setup" action="" method="post">
	        ...
	        <li><a href="#groupblog_landing_page"><?php _e('Umleiten', 'groupblog'); ?></a></li>
	        <li><a href="#groupblog_template_layout"><?php _e('Layout', 'groupblog'); ?></a></li>
	        <li><a href="#groupblog_default_blog_settings"><?php _e('Standardeinstellungen', 'groupblog'); ?></a></li>
	        <li><a href="#groupblog_validation_settings"><?php _e('Validierung', 'groupblog'); ?></a></li>
	        <li><a href="#groupblog_about"><?php _e('Über', 'groupblog'); ?></a></li>
	      </ul>
				...
				<div id='groupblog_default_theme'>
					...
					*/
					...
					<h3><?php _e( 'Standardtheme', 'groupblog' ) ?></h3>

					<div id="select-theme">
						<label for="theme"><?php _e( 'Wähle das Standarddesign für neue Gruppenblogs aus:', 'groupblog' ) ?></label>
						<select id="theme" name="theme" size="1">

						<optgroup label="<?php echo esc_attr( __( 'GruppenBlog Themes:', 'groupblog' ) ) ?>">
					 	<?php
						...
						if ( !empty( $groupblog_themes_options ) ) {
							...
							echo '<option value="" disabled="disabled">' . __( 'Keine GruppenBlog-fähigen Themes verfügbar', 'groupblog' ) . '</option>';
						}

						?>
						</optgroup>
						<optgroup label="<?php echo esc_attr( __( 'Reguläre Themes:', 'groupblog' ) ) ?>">
						<?php
						...
						if ( !empty( $non_groupblog_themes_options ) ) {
							...
							echo '<option value="" disabled="disabled">' . __( 'Keine regulären Themes verfügbar', 'groupblog' ) . '</option>';
						}

						?>
						</optgroup>

						<option value="" <?php selected( $current_groupblog_theme, '' ) ?>><?php _e( '- Nichts ausgewählt -', 'groupblog' ) ?></option>
					...
					<?php if ( !empty( $current_groupblog_theme ) ) : 
						...
						<div id="current-theme"<?php echo $wp3point4classes; ?>>
							<?php if ( $theme_has_screenshot ) : ?>
								<img src="<?php echo $themes[$current_groupblog_theme]['Theme Root URI'] . '/' . $themes[$current_groupblog_theme]['Stylesheet'] . '/' . $themes[$current_groupblog_theme]['Screenshot']; ?>" alt="<?php _e('Aktuelle Themevorschau'); ?>" />
							<?php endif; ?>

							<div class="alt" id="current-theme-info">
								<h4><?php	/* translators: 1: theme title, 2: theme version, 3: theme author */
								printf(__('%1$s %2$s von %3$s'), $themes[$current_groupblog_theme]['Title'], $themes[$current_groupblog_theme]['Version'], $themes[$current_groupblog_theme]['Author']) ; ?></h4>
				...
				<div id='groupblog_landing_page'>
					...
					<h3><?php _e( 'Standard-Landing Page', 'groupblog' ) ?></h3>

					<p><?php _e( 'Die Seite, auf die über die Registerkarte "Blog" der Gruppennavigation verwiesen wird. Wenn Du "Deaktiviert" auswählst, wird die im Plugin enthaltene Buddypress-Vorlage verwendet. Es findet keine Umleitung statt. Die Einstellung "Startseite" erstellt eine Weiterleitung zur Blog-Startseite. Mit der Einstellung "Vorlagenseite" wird eine Umleitung zur Blogvorlagenseite erstellt. Wenn Du diese Einstellung verwendest, kannst Du auf der nächsten Registerkarte ein bestimmtes Seitenvorlagenlayout auswählen.', 'groupblog' ) ?></p>
					<table class="form-table">
						<tbody>
						<tr>
							<th><?php _e( 'Weiterleitung aktiviert zu:', 'groupblog' ) ?></th>
							<td>
								<label><input class="info-off" name="bp_groupblog_redirect_blog" id="bp_groupblog_redirect_blog"  value="0" type="radio" <?php if ($opt['redirectblog']== 0) echo 'checked="checked"'; ?> > <?php _e( 'Deaktiviert', 'groupblog' ) ?></label>
							</td>
						</tr>
						<tr>
							<th></th>
							<td>
								<label><input class="info-off" name="bp_groupblog_redirect_blog" id="bp_groupblog_redirect_blog"  value="1" type="radio" <?php if ($opt['redirectblog']== 1) echo 'checked="checked"'; ?> > <?php _e( 'Startseite', 'groupblog' ) ?></label>
							</td>
						<tr>
							<th></th>
							<td>
								<label><input class="info-on" name="bp_groupblog_redirect_blog" id="bp_groupblog_redirect_blog"  value="2" type="radio" <?php if ($opt['redirectblog']== 2) echo 'checked="checked"'; ?> > <?php _e( 'Titel der Seitenvorlage: ', 'groupblog' ) ?></label>
								<input name="bp_groupblog_page_title" id="bp_groupblog_page_title" value="<?php echo $opt['pagetitle'];?>" size="10" type="text" />
								<span class="notice" id="redirect_notice" style="display:none;"> <?php _e( 'Alle vorhandenen GruppenBlogs werden bei jeder Änderung automatisch aktualisiert.', 'groupblog' ) ?></span>
								<p class="info"><?php _e( 'Mit der Option "Vorlagenseite" wird eine Seite in Gruppenblogs und Links zu einer Vorlagendatei in Deinem Theme erstellt. Mache Dir keine Sorgen um den von Dir gewählten Namen, wir stellen sicher, dass Deine Seite den Weg zur Vorlagendatei findet. Stelle für benutzerdefinierte Designs sicher, dass <a href="http://codex.wordpress.org/Pages#Creating_Your_Own_Page_Templates">diese Vorlagendatei manuell erstellt wird</a>.', 'groupblog' ) ?>
								</p>
							</td>
						</tr>
						<tr>
							<th><?php _e( 'Zur Gruppenstartseite umleiten:', 'groupblog' ) ?></th>
							<td>
								<label for="deep_group_integration"><input name="deep_group_integration" type="checkbox" id="deep_group_integration" value="1" <?php if ($opt['deep_group_integration'] == 1) echo('checked="checked"'); ?> /> <?php _e( 'Ja, leite Gruppenstartseite zur Blogstartseite um','groupblog') ?></label><p><?php _e('Diese Option übernimmt die Kontrolle über die GROUP-Homepage und leitet sie zur BLOG-Homepage weiter. Dies aktiviert Posten aus der Gruppenstartseite mit P2 anstelle des BuddyPress-Formulars.', 'groupblog' ) ?></p>
					  	</td>
						</tr>
						</tbody>
					</table>
				...
				<?php if ( $opt['theme'] == 'p2|p2-buddypress' )	{ ?>
					...
					<h3><?php _e( 'Layout der Vorlagenseite', 'groupblog' ) ?></h3>
		      ...
		      <h3><?php _e( 'Default Blog Settings', 'groupblog' ) ?></h3>
				...
				<div id='groupblog_validation_settings'>
				    ...
				    <tr>
						...
						<tr>
							<th><?php _e( 'Minimale Länge:', 'groupblog' ) ?></th>
							<td>
				...
				<div id='groupblog_about'>
					...
					<h3><?php _e( 'Über dieses Plugin', 'groupblog' ) ?></h3>

			    <div>
			    	<span class="indent"><strong><?php _e( 'Autor', 'groupblog' ) ?></strong></span>
