...
require 'inc/psource-plugin-update/plugin-update-checker.php';
$MyUpdateChecker = Puc_v4_Factory::buildUpdateChecker(
	'https://n3rds.work//wp-update-server/?action=get_metadata&slug=bp-groupblog', 
	__FILE__, 
	'bp-groupblog' 
);
...
function bp_groupblog_setup() {
	...
	$groupblog_blogdefaults = array(
		...
		'default_cat_name' => 'Unkategorisiert',
...
function groupblog_edit_settings() {
	...
	if ( !bp_groupblog_blog_exists( $group_id ) ) {
		if ( isset( $_POST['groupblog-enable-blog'] ) ) {
			...
			//They forgot to choose a blog, so send them back and make them do it!
				bp_core_add_message( __( 'Bitte wähle einen Deiner Blogs aus dem Dropdown-Menü.' . $group_id, 'groupblog' ), 'error' );
	...
	if ( !groupblog_edit_base_settings( $settings['groupblog-enable-blog'], $settings['groupblog-silent-add'], $settings['default-administrator'], $settings['default-moderator'], $settings['default-member'], $settings['page_template_layout'], $group_id, $groupblog_blog_id ) ) {
		bp_core_add_message( __( 'Beim Erstellen Deines Gruppenblogs ist ein Fehler aufgetreten. Versuche es erneut.', 'groupblog' ), 'error' );
	} else {
		bp_core_add_message( __( 'Gruppendetails wurden erfolgreich aktualisiert.', 'groupblog' ) );
	}
...
function bp_groupblog_show_blog_form( $blogname = '', $blog_title = '', $errors = '' ) {
	...
	<?php if ( !$groupblog_create_screen && $blog_id != '' ) : ?>
		...
		<label for="blog_title"><strong><?php _e( 'Blog Titel:', 'groupblog' ) ?></strong></label>
		...
		<label for="blogname"><strong><?php _e( 'Blog Addresse:', 'groupblog' ) ?></strong></label>
		...
		<div id="uncouple-blog">
			<label for="uncouple"><?php printf( __( 'Entkoppele den Blog "%1$s" von der Gruppe "%2$s":', 'groupblog' ), $blog_details->blogname, $bp->groups->current_group->name ) ?></label>

			<p class="description"><?php printf( __( '<strong>Hinweis:</strong> Durch das Entkoppeln wird der Blog aus der Navigation Deiner Gruppe entfernt und die zukünftige Synchronisierung von Gruppenmitgliedern und Blogautoren verhindert. Es werden jedoch keine Änderungsblogberechtigungen für aktuelle Mitglieder entfernt. Besuche <a href="%1$s">das Benutzerfenster</a>, wenn Du Benutzer aus dem Blog entfernen möchtest.', 'groupblog' ), $blog_details->siteurl . '/wp-admin/users.php' ) ?></p>

			<a class="button" href="<?php echo wp_nonce_url( bp_get_group_permalink( $bp->groups->current_group ) . 'admin/group-blog/uncouple', 'groupblog-uncouple' ) ?>"><?php _e( 'Entkoppeln', 'groupblog' ); ?></a>
	...
	<?php else : ?>
		...
		<p><?php _e( 'Wähle entweder einen Deiner vorhandenen Blogs aus oder erstelle einen neuen Blog mit den unten angezeigten Details.', 'groupblog' ); ?><br /><?php _e('Sei vorsichtig, da Du nur einmal wählen kannst. Später kannst Du den Blog möglicherweise noch deaktivieren oder aktivieren, aber Deine Auswahl ist festgelegt.', 'groupblog' ); ?></p>

		<p>
			<label for="groupblog-create-new-no"><input type="radio" value="no" name="groupblog-create-new" id="groupblog-create-new-no" /><span>&nbsp;<?php _e( 'Verwende einen Deiner eigenen verfügbaren Blogs:', 'groupblog' ); ?>&nbsp;</span>

			<?php $user_blogs = get_blogs_of_user( get_current_user_id() ) ?>

			<select name="groupblog-blogid" id="groupblog-blogid">
				<option value="0"><?php _e( 'Wähle einen Blog', 'groupblog' ) ?></option>
				<?php
		...
		<p>
			<label for="groupblog-create-new-yes"><input type="radio" value="yes" name="groupblog-create-new" id="groupblog-create-new-yes" checked="checked" /><span>&nbsp;<?php _e( 'Oder erstelle einen neuen Blog', 'groupblog' ); ?></label></span>
		</p>

		<ul id="groupblog-details">
			<li>
				<label class="groupblog-label" for="blog_title"><strong><?php _e( 'Blog Titel:', 'groupblog' ) ?></strong></label>
			...
			<li>
				<label class="groupblog-label" for="blogname"><strong><?php _e( 'Blog Addresse:', 'groupblog' ) ?></strong></label>
				<?php if ( $errmsg = $errors->get_error_message('blogname') ) : ?>
					<span class="error"><?php echo $errmsg ?></span>
				<?php endif ?>
	...
	foreach ($errors->errors as $key => $value) {
		...
		if ($key == 'blogname'){
			...
			foreach ($value as $subkey => $subvalue) {
				...
				switch ($subvalue){
					case 'Es sind nur Kleinbuchstaben (a-z) und Zahlen zulässig.':
						...
						if( $result['blogname'] != $maybe[0] ) {
							...
							$newerrors->add('blogname', __("Es sind nur Kleinbuchstaben (a-z) und Zahlen zulässig", 'groupblog'));

						}
						break;
					case 'Der Webseiten-Name muss mindestens 4 Zeichen lang sein.':
						if( strlen( $result['blogname'] ) < $checks[minlength] && !is_super_admin() )
						$newerrors->add('blogname',  __("Der Blogname benötigt mindestens " . $checks[minlength] . " Zeichen", 'groupblog'));
						break;
					case "Leider enthalten Webseiten-Namen möglicherweise nicht das Zeichen &#8220;_&#8221;!":
						if($checks['allowunderscores']!= 1) {
							$newerrors->add('blogname', __("Entschuldigung, Blog-Namen enthalten möglicherweise nicht das Zeichen '_'!", 'groupblog'));
						}
						break;
					case 'Entschuldigung, Webseiten-Namen müssen auch Buchstaben haben!':
						if($checks['allownumeric'] != 1){
							$newerrors->add('blogname', __("Entschuldigung, Blog-Namen müssen auch Buchstaben haben!", 'groupblog'));
						}
						break;
					default:
...
function bp_groupblog_process_uncouple() {
	if ( bp_is_group() && bp_is_current_action( 'admin' ) && bp_is_action_variable( 'group-blog', 0 ) && bp_is_action_variable( 'uncouple', 1 ) ) {
		...
		if ( !bp_group_is_admin() ) {
			bp_core_add_message( __( 'Du musst ein Gruppenadministrator sein, um diese Aktion ausführen zu können.', 'groupblog' ), 'error' );
		...
		bp_core_add_message( __( 'Blog entkoppelt.', 'groupblog' ) );
...
function bp_groupblog_signup_blog($blogname = '', $blog_title = '', $errors = '') {
	...
	<?php } ?>
		...
		<div class="checkbox">
			<label><input type="checkbox" name="groupblog-enable-blog" id="groupblog-enable-blog" value="1"<?php bp_groupblog_show_enabled( $group_id ) ?>/> <?php _e( 'Gruppenblog aktivieren', 'groupblog' ); ?></label>
		</div>
		...
		<div id="groupblog-member-options">
			...
			<h3><?php _e( 'Mitgliederoptionen', 'groupblog' ) ?></h3>

			<p><?php _e( 'Aktiviere das Posten von Blogs, um das Hinzufügen von Gruppenmitgliedern zum Blog mit den unten festgelegten Rollen zu ermöglichen.', 'groupblog' ); ?><br /><?php _e( 'Wenn deaktiviert, werden alle Mitglieder vorübergehend auf Abonnenten eingestellt, wodurch das Posten deaktiviert wird.', 'groupblog' ); ?></p>

			<div class="checkbox">
				<label><input type="checkbox" name="groupblog-silent-add" id="groupblog-silent-add" value="1"<?php if ( bp_groupblog_silent_add( $group_id ) ) { ?> checked="checked"<?php } ?>/> <?php _e( 'Aktiviere das Blog-Posting für Mitglieder', 'groupblog' ); ?></label>
			</div>
			...
			<label><strong><?php _e( 'Standardadministratorrolle:', 'groupblog' ); ?></strong></label>
			...
			<input type="radio" <?php checked( $groupblog_default_admin_role, 'editor' ) ?> value="editor" name="default-administrator" /><span>&nbsp;<?php _e( 'Redakteur', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_admin_role, 'author' ) ?> value="author" name="default-administrator" /><span>&nbsp;<?php _e( 'Autor', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_admin_role, 'contributor' ) ?> value="contributor" name="default-administrator" /><span>&nbsp;<?php _e( 'Mitwirkender', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_admin_role, 'subscriber' ) ?> value="subscriber" name="default-administrator" /><span>&nbsp;<?php _e( 'Abonnent', 'groupblog' ); ?>&nbsp;&nbsp;</span>

			<label><strong><?php _e( 'Standard-Moderatorrolle:', 'groupblog' ); ?></strong></label>
			...
			<input type="radio" <?php checked( $groupblog_default_mod_role, 'editor' ) ?> value="editor" name="default-moderator" /><span>&nbsp;<?php _e( 'Redakteur', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_mod_role, 'author' ) ?> value="author" name="default-moderator" /><span>&nbsp;<?php _e( 'Autor', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_mod_role, 'contributor' ) ?> value="contributor" name="default-moderator" /><span>&nbsp;<?php _e( 'Mitwirkender', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_mod_role, 'subscriber' ) ?> value="subscriber" name="default-moderator" /><span>&nbsp;<?php _e( 'Abonnent', 'groupblog' ); ?>&nbsp;&nbsp;</span>

			<label><strong><?php _e( 'Standardmitgliedsrolle:', 'groupblog' ); ?></strong></label>
			...
			<input type="radio" <?php checked( $groupblog_default_member_role, 'editor' ) ?> value="editor" name="default-member" /><span>&nbsp;<?php _e( 'Redakteur', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_member_role, 'author' ) ?> value="author" name="default-member" /><span>&nbsp;<?php _e( 'Autor', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_member_role, 'contributor' ) ?> value="contributor" name="default-member" /><span>&nbsp;<?php _e( 'Mitwirkender', 'groupblog' ); ?>&nbsp;&nbsp;</span>
			<input type="radio" <?php checked( $groupblog_default_member_role, 'subscriber' ) ?> value="subscriber" name="default-member" /><span>&nbsp;<?php _e( 'Abonnent', 'groupblog' ); ?>&nbsp;&nbsp;</span>

			<div id="groupblog-member-roles">
				<label><strong><?php _e( 'Ein bisschen über Mitgliedsrollen:', 'groupblog' ); ?></strong></label>
				<ul id="groupblog-members">
					<li><?php _e( 'Administrator', 'groupblog' ); ?> - <?php _e( "Jemand, der Zugriff auf alle Verwaltungsfunktionen hat.", 'groupblog' ); ?></li>
					<li><?php _e( 'Redakteur', 'groupblog' ); ?> - <?php _e( "Jemand, der Beiträge veröffentlichen, Beiträge verwalten sowie Beiträge anderer Personen verwalten kann usw.", 'groupblog' ); ?></li>
					<li><?php _e( 'Autor', 'groupblog' ); ?> - <?php _e( "Jemand, der seine eigenen Beiträge veröffentlichen und verwalten kann.", 'groupblog' ); ?></li>
					<li><?php _e( 'Mitwirkender', 'groupblog' ); ?> - <?php _e( "Jemand, der seine Beiträge schreiben und verwalten kann, aber keine Beiträge veröffentlicht.", 'groupblog' ); ?></li>
					<li><?php _e( 'Abonnent', 'groupblog' ); ?> - <?php _e( "Jemand, der Kommentare lesen/kommentieren/Newsletter erhalten kann usw.", 'groupblog' ); ?></li>
				</ul>
			</div>
		...
		<?php if ( bp_groupblog_allow_group_admin_layout() )	: ?>
			...
			<div id="groupblog-layout-options">
				...
				<h3><?php _e( 'Wähle Layout', 'groupblog' ) ?></h3>

				<p class="enabled"><?php _e( 'Bitte wähle ein Layout aus, das Du für Deinen Gruppenblog verwenden möchtest.', 'groupblog' ) ?></p>
				...
				<table class="enabled" id="availablethemes" cellspacing="0" cellpadding="0">
					...
					<tr>
						<td class="available-theme top left">
							...
							<p class="description"><?php _e( 'Ausgewogene Vorlage für Gruppen mit verschiedenen Postings.', 'groupblog' ) ?></p>
						</td>
						<td class="available-theme top">
							...
							<p class="description"><?php _e( 'Ideal für die einfache Auflistung von Posts in chronologischer Reihenfolge.', 'groupblog' ) ?></p>
						</td>
					</tr>
					</tbody>
				</table>
		...
		<?php if ( !$groupblog_create_screen ) : ?>
			<p>
				<input id="save" type="submit" name="save" class="submit" value="<?php _e('Änderungen speichern &raquo;', 'groupblog') ?>"/>
			</p>
		<?php endif;
}
...
function bp_groupblog_validate_blog_signup() {
  ...
  // Re-validate user info.
	...
	if ( $errors->get_error_code() ) {
		...
		$message .= __( ' Wir empfehlen, die unten stehende Blog-Adresse gemäß den folgenden Anforderungen anzupassen:', 'groupblog' ) . '<br />';
		if ( $checks['allowunderscores'] != '1' || $checks['allowdashes'] != '1' )
			$message .= __( ' &raquo; Nur Buchstaben und Zahlen erlaubt.', 'groupblog' ) . '<br />';
		$message .= sprintf( __( ' &raquo; Muss mindestens %s Zeichen enthalten.', 'groupblog' ), $checks['minlength'] ) . '<br />' ;
		if ( $checks['allownumeric'] != '1' )
			$message .= __( ' &raquo; Muss auch Buchstaben enthalten.', 'groupblog' );
...
function bp_groupblog_set_group_to_post_activity( $activity, $args = array() ) {
	...
	if ( ! empty( $id ) ) {
		...
		$activity->action = sprintf( __( '%s hat den Blog-Beitrag %s in der Gruppe %s bearbeitet:', 'groupblog'), bp_core_get_userlink( $post->post_author ), '<a href="' . get_permalink( $post->ID ) .'">' . $post->post_title . '</a>', '<a href="' . bp_get_group_permalink( $group ) . '">' . esc_attr( $group->name ) . '</a>' );

	// This is a new blog post!
	} else {
		$activity->action = sprintf( __( '%s hat einen neuen Blog-Beitrag %s in der Gruppe %s geschrieben:', 'groupblog'), bp_core_get_userlink( $post->post_author ), '<a href="' . get_permalink( $post->ID ) .'">' . $post->post_title . '</a>', '<a href="' . bp_get_group_permalink( $group ) . '">' . esc_attr( $group->name ) . '</a>' );
	}
...
add_action( 'bp_register_activity_actions', function() {
	bp_activity_set_action(
		...
		__( 'Neuer Groupblog-Kommentar', 'bp-groupblog' ),
		'bp_groupblog_format_activity_action_new_groupblog_comment',
		bp_is_user() ? __( 'Groupblog Kommentare', 'bp-groupblog' ) : __( 'Blog-Kommentare', 'bp-groupblog' ),
...
function bp_groupblog_format_activity_action_new_groupblog_comment( $action, $activity ) {
	...
	if ( empty( $activity->user_id ) && ! empty( $anonymous ) ) {
		...
		$user_link = esc_html__( 'Anonymer Benutzer', 'bp-groupblog' );
	}

	// Build the complete activity action string.
	$action = sprintf( __( '%1$s kommentierte den Beitrag %2$s, im Gruppenblog %3$s', 'buddypress' ), $user_link, $post_link, '<a href="' . esc_url( $blog_url ) . '">' . esc_html( $blog_name ) . '</a>' );
